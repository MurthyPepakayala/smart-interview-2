...
const InterviewChat = ({ role, difficulty, jobDescription, onFinish }: InterviewChatProps) => {
  ...
  useEffect(() => {
    ...
    if (lastMsg?.role === "ai" && lastMsg.text !== lastSpokenRef.current) {
      ...
      speak(lastMsg.text, () => {
        if (!isComplete) startListening();
      });
    }
  }, [messages, isTyping, speak, isComplete]);
  ...
  const handleSend = useCallback((overrideInput?: string) => {
    ...
    stopListening();
    const userMsg: Message = { id: messages.length, role: "user", text: textToSend, timestamp: new Date() };
  ...
  }, [input, isTyping, messages, stopListening]);
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      setInput(fullText);
      if (fullText.trim()) resetSilenceTimer();
    };
